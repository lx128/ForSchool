...
namespace RentreeGPAL
{
	...
	public partial class MainForm : Form
	{
		...
		{
			string line,str, slogin, slogin2,
			...
			File.Delete("eleve/Adele Eleves.csv");
			...
			FonctionFichierBinaireCLR adele_ent = new FonctionFichierBinaireCLR("eleve/Adele Eleves.csv");//Liste de codes
...
//									myCLR.element(line,0, out delta_id, '\n');
						...
						//fichier pour adele
						adele_ent.EcrireLigne(snomsco+";"+sprenomsco+";"+sinesco+";"+sbirth+";"+sclassesco+";"+slogin2+";"+smdp+"\n");
	...
	/*						MessageBox.Show(snom+"	"+sprenom+"	"+slogin2+"	"+smdp+"	"+sbirth+"	"+sclasse+"	"
		...
		*/				}
		...
		void Button2Click(object sender, EventArgs e)//profs
		{
			...
			StreamReader deltar;
			StreamWriter deltaw;
			File.Delete("prof/Adele Profs.csv");
			...
			FonctionFichierBinaireCLR adele = new FonctionFichierBinaireCLR("prof/Adele Profs.csv");//Adele
...
//Nom,Prénom,Identifiant,Mot de passe
			...
			while (!profs.EndOfStream)
			{
				...
				//Fichier pour adele
				adele.EcrireLigne(slogin2+"#"+smdp+","+myCLR.Remplace(snom)+","+myCLR.Remplace(sprenom)+"\n");
			}
			label1.Text=comp2.ToString()+" entrées traités";
			profs.Close();

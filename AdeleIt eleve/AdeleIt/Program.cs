...
namespace AdeleIt
{
	class Program
	{
		...
		{
			string login="eleve.test", classe="3DP6", classesol="401", str, prenom, nom, login2;
			int comp;
			...
			Directory.Move("c:\\Users\\classes\\"+classesol+"\\eleves\\"+login,"c:\\Adele2k3\\eleves\\"+classe+"\\"+login2);
...
//			DirEntry.Properties["cn"].Value = str;
			DirEntry.Properties["description"].Value = classe;
			...
			DirEntry.Properties["homeDirectory"].Value = "\\\\DC1\\eleves$\\"+classe+"\\"+login2;
			DirEntry.Properties["homePhone"].Value = "S"+(comp+1).ToString();
			DirEntry.Properties["mail"].Value = "@xxxx";
//			DirEntry.Properties["name"].Value = "U:";
			DirEntry.Properties["pager"].Value = comp.ToString();
			DirEntry.Properties["profilePath"].Value = "\\\\DC1\\profils$\\eleves";
			DirEntry.Properties["scriptPath"].Value = "eleve.vbs";
			...
			DirEntry.Properties["wWWHomePage"].Value = classe;
			DirEntry.Properties["userPrincipalName"].Value = login+"@xxxx.local";
//			Console.WriteLine(DirEntry.Properties["name"].Value);

			//changement des droits
			DirectoryEntry TestGroup = new DirectoryEntry("LDAP://DC1/CN=g_eleves,CN=Users,DC=xxxx");//,DC=local");
			TestGroup.Properties["member"].Add(DirEntry.Properties["distinguishedName"].Value);
			TestGroup.CommitChanges();
			TestGroup = new DirectoryEntry("LDAP://DC1/CN=classe-"+classe+",OU=groupes,DC=xxxx");//,DC=local");
			TestGroup.Properties["member"].Add(DirEntry.Properties["distinguishedName"].Value);
			TestGroup.CommitChanges();
			TestGroup.Close();
			...
			//déplacement dans AD
			DirEntry.MoveTo(new DirectoryEntry("LDAP://DC1/OU=eleves,DC=xxxx"));//,DC=local");
			DirEntry.Close();
			...
			child.Rename("CN=" + nom+" "+prenom+"    "+comp.ToString());
